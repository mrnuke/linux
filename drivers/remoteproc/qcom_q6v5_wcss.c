...
static int ipq9574_probe_clocks(struct q6v5_wcss *wcss)
{
	const char *q6_clks[] = { "anoc_wcss_axi_m", "q6_axim", "q6_ahb",
				  "q6_ahb_s","q6ss_boot", "wcss_ecahb",
				  "wcss_acmt", "mem_noc_q6_axi", "wcss_q6_tbu",
				  "sys_noc_wcss_ahb" };
	const char *clks[] = { "q6_axim2", "wcss_ahb_s", "wcss_axi_m" };
	int i, ret;
...
static int q6v5_wcss_probe(struct platform_device *pdev)
{
	...
	case WCSS_QCS404:
		...
		break;
	case WCSS_IPQ9574:
		ret = ipq9574_probe_clocks(wcss);
		if (ret)
			return ret;

		dev_err(&pdev->dev, "Habemus %d q6 clk\n", wcss->num_q6_clks);
		dev_err(&pdev->dev, "Habemus %d clocks\n", wcss->num_clks);
	}
